...
router.get('/sake', async(req, res) => {
    ...
    const sql = `
    SELECT cs.*, cm.mark_id, cm.cart_sake_id, mark.pics, ps.pro_name, ps.pro_img, pf.pro_price, pf.pro_capacity, pf.pro_gift, pf.pro_mark 
    FROM cart_sake cs 
...
router.delete('/sake', async(req, res) =>{
    ...
    const sql_sake = "DELETE FROM `cart_sake` WHERE `cart_sake_id`=? AND `member_id`=? AND `pro_id`=?"
    const sql_mark = "DELETE FROM `cart_mark` WHERE `cart_sake_id`=?"
    const [result_sake] = await db.query(sql_sake, [cart_sake_id, member_id, pro_id]);
...
router.put('/sake', async(req, res) =>{
    ...
    const cart_quantity = req.body.cart_quantity ? parseInt(req.body.cart_quantity) : 'no quantity'
    const member_id = req.body.member_id ? parseInt(req.body.member_id) : 'no member_id'
    const cart_sake_id = req.body.cart_sake_id ? req.body.cart_sake_id : '0'
    const pro_id = req.body.pro_id ? parseInt(req.body.pro_id) : '0'
    const sql = "UPDATE `cart_sake` SET `cart_quantity`=? WHERE `cart_sake_id`=? AND `member_id`=? AND `pro_id`=?;"
...
router.delete('/gift', async(req, res) =>{
    ...
    const sql_gift = "DELETE FROM `cart_gift` WHERE `cart_gift_id`=? AND `member_id`=?"
    ...
    if(!! result_gift.affectedRows){
        ...
        output.success = [(!! result_gift.affectedRows), (!!result_gift_d_d.affectedRows)];
...
router.put('/gift', async(req, res) =>{
    ...
    const member_id = req.body.member_id ? parseInt(req.body.member_id) : 'no member_id'
    const cart_gift_id = req.body.cart_gift_id ? req.body.cart_gift_id : '0'
    const sql = "UPDATE `cart_gift` SET `cart_quantity`=? WHERE `cart_gift_id`=? AND `member_id`=?"
    const [result] = await db.query(sql, [
        cart_quantity,
        cart_gift_id,
        member_id,
